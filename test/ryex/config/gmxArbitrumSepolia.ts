 ...
 */
/**
 * GMX v2 Arbitrum Sepolia 마켓 주소 조회:
 *   node -e "require('ethers').JsonRpcProvider('https://sepolia-rollup.arbitrum.io/rpc')
 *     .then(p => new ethers.Contract(READER, [...], p).getMarkets(DATA_STORE, 0, 30))"
 *
 * 마켓 토큰 = GMX market 풀 ERC-20 주소 (createOrder 시 market 파라미터로 전달)
 * 인덱스 토큰:
 *   MARKET_ETH → WETH 0x980B62Da83eFf3D4576C647993b0c1D7faf17c73 (18dec)
 *   MARKET_BTC → BTC  0xF79cE1Cf38A09D572b021B4C5548b75A14082F12 (8dec)
 */
export const GMX = {
  // ── 핵심 저장소 ──────────────────────────────────────────────────────────
  ...
  // ── GMX 마켓 토큰 (Reader.getMarkets로 확인) ──────────────────────────────
